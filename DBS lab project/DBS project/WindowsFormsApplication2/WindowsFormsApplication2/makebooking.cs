...
namespace WindowsFormsApplication2
{
    public partial class makebooking : Form
    {
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            conn.Open();
            OracleCommand comm = new OracleCommand();
            comm.CommandText = "select arrival_time from TRAIN_SCHEDULE where train_id = '" + train + "' and day = '" + day + "'";
            comm.CommandType = CommandType.Text;
            DataSet ds = new DataSet();
            OracleDataAdapter da = new OracleDataAdapter(comm.CommandText, conn);
            da.Fill(ds, "Tbl_train_schedule");
            ...
            int t = dt.Rows.Count;
            DataRow dr = dt.Rows[j];
            time = dr["arrival_time"].ToString();
            textBox4.Text = dr["arrival_time"].ToString();
            

            OracleConnection conn1 = new OracleConnection(oradb);
            conn1.Open();
            OracleCommand comm1 = new OracleCommand();
            comm1.CommandText = "select available from TRAIN where train_id = '" + train + "'";
            ...
            int c = int.Parse(dr1["available"].ToString());
            if (c == 0)
